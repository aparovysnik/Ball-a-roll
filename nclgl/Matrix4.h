...
array in a neater way might be a good 
, as the floats that make the matrix 
...
public:
	...
	Vector3 GetForwardVector() {
		Vector3 fw = GetColumn(2);
		fw.z = 0 - fw.z;
		return fw;
	}

	Vector3 GetCamUpVector() {
		Vector3 up = GetColumn(1);
		up.z = 0 - up.z;
		return up;
	}

	Vector3 GetCamLeftVector() {
		Vector3 lf = GetColumn(0);
		lf.z = 0 - lf.z;
		return lf;
	}
	...
	static Matrix4 Rotation(Vector4 quat);
